...
### Level-boundary review rule

Before revising a chapter, map each section to one reader question: Level 1 asks what problem the method solves; Level 2 asks what happens in one concrete case; Level 3 asks how to state, calculate, or implement that case; and Level 4 asks what changes with scale, evidence, deployment, or system constraints. Move or cut material that answers the same question twice. Do not introduce batch shapes, framework APIs, detailed activation comparisons, layer composition, or systems constraints in Level 2 when they belong to Levels 3 or 4. Do not use Level 4 to provide the conceptual bridge owned by the next chapter.
...
- **Chapters 1-49 — close pedagogical rewrite — active.** The first navigation-and-transition pass did not provide a sufficiently natural learning narrative. Re-review every chapter at paragraph level, starting with Chapter 1, against a stricter standard: begin with the ordinary decision or observation before technical vocabulary; introduce a term only after its referent is clear; define each term in plain language before relying on it; make each paragraph answer a reader's immediate question and establish the next one; move qualifications to the point at which the reader can use them; and remove table-led, code-led, generic, or fabricated framing that obscures the underlying idea. Preserve mathematical and empirical precision, but do not treat a clear heading, a concept box, or a figure caption as a substitute for explanation in the prose. Use the fully revised Chapter 1 as the agreed reference before applying the standard sequentially across the remaining 48 chapters.
- **Chapter 1 — close pedagogical rewrite — in progress.** Replaced the table-led organisational opening with the ordinary outreach decision and distinguished the household identifier, information available before contact, and outcome observed later before introducing the terms `feature`, `target`, and `label`. Rewrote the Level 1 comparison of hand-written and fitted rules, the Level 2 entry and definitions, the explanation of baselines and evaluation, the Level 3 explanations of linear regression, residuals, and thresholds, and the Level 4 deployment boundaries. Source checks after this pass: four level headings; 22 balanced code fences; 9/9 figure captions with alt text; resolving Chapter 2 link; resolving bibliography keys; and a clean whitespace check. Rendered reader review is pending before this becomes the standard for the remaining chapters.
- **Chapters 3 and 5 — level-boundary correction — completed source pass; rendered review pending.** Chapter 3 now keeps the training cycle and local direction in Level 2, while Level 3 owns the derivative and update notation; it also removes the obsolete downhill-analogy reference. Chapter 5 keeps one-neuron arithmetic, bias, and the basic activation hand-off in Level 2; batch application and tensor shapes now begin in Level 3; and the Level 4 layer derivation moves to Chapter 6, leaving Level 4 for scale and systems constraints. These corrections establish the level-boundary review rule for subsequent chapter revisions.
- **Chapter 6 — close pedagogical and level-boundary rewrite — completed source pass; rendered review pending.** Replaced the matrix-first opening with the unresolved need for several intermediate values from the same household information. Delayed the weight matrix, bias vector, output notation, broadcasting, and batch shapes until the reader has seen the three concrete output calculations. Moved the concept box after the first layer diagram; rewrote Level 1 around a layer as several separate learned calculations; moved the batch figure and its shape explanation from Level 2 to Level 3; and retained Level 4 for matrix-multiplication scale, memory, parallelism, and structured alternatives to dense layers. Source checks: four level headings; 20 balanced code fences; 5/5 figure captions with alt text; resolving local links and bibliography keys; and a clean whitespace check.
- **Chapter 7 — close pedagogical and level-boundary rewrite — completed source pass; rendered review pending.** Replaced the matrix-first account of non-linearity with the concrete failure of stacking straight-rule calculations, then introduced ReLU as a value-by-value change that produces distinct input regions. Delayed the concept box until after the first visual; removed premature calibration qualifications from Level 1; retained only the ReLU forward step in Level 2; and moved activation comparisons, gradient transmission, output-role distinctions, and affine-collapse derivations into Level 3. Level 4 remains responsible for empirical activation trade-offs, memory traffic, numerical precision, and kernel fusion. Source checks: four level headings; 20 balanced code fences; 5/5 figure captions with alt text; resolving local links and bibliography keys; and a clean whitespace check.
- **Chapter 8 — close pedagogical and level-boundary rewrite — completed source pass; rendered review pending.** Replaced the component catalogue with the immediate problem of connecting earlier predictions to the parameters used for later ones. Reframed the synthetic curves as a transparent setting for the loop and delayed the concept box until after the loop visual. Level 1 now names scores and recorded outcomes before logits, probabilities, or numerical-stability details; Level 2 follows one observation through forward and backward calculations; and Level 3 now begins with mini-batch iteration, evaluation mode, gradient storage, checkpoint state, batch equations, and executable implementation. Level 4 retains stochastic-optimisation evidence, batch trade-offs, precision, distributed training, and production audit requirements. Source checks: four level headings; 20 balanced code fences; 6/6 figure captions with alt text; resolving local links and bibliography keys; and a clean whitespace check.
- **Chapter 9 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the abstract training-versus-deployment opening with the unresolved status of the promising held-out result in Chapter 8. Delayed the generalisation concept box until after the fixed-capacity visual; rewrote Level 1 around the difference between a recurring pattern and sample-specific detail; replaced the early terminology-first discussion of held-out data with the ordinary need for new observations; and moved the precise training, validation, test, and held-out distinction to Level 2, immediately before its workflow diagram. Levels 3 and 4 retain the risk, repeated-sampling, cross-validation, shift, contamination, and modern-capacity evidence required for their respective questions. Source checks: four level headings; 14 balanced code fences; 6/6 figure captions with alt text; resolving local links and bibliography keys; and a clean whitespace check.
- **Chapter 10 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the catalogue of regularisation methods with the concrete degree-15 fit from Chapter 9 and the choice to change the preferred fit without removing polynomial terms. Rewrote the ridge example and Level 1 so that penalties, dropout, augmentation, and early stopping appear as distinct fitting preferences only after the reader has seen the changed fit. Removed early broad exclusions and moved the evaluation boundary to a concise Level 1 reminder that regularisation remains a development choice governed by validation and test roles. The existing mechanics, formal derivations, framework contracts, and systems evidence retain their Level 2–4 ownership. Source checks: four level headings; 16 balanced code fences; 5/5 figure captions with alt text; resolving local links and bibliography keys; and a clean whitespace check.
- **Chapter 11 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the regularisation-first, systems-heavy opening with the concrete state of a network before it has seen its first training batch: starting values must distinguish hidden units and allow information to traverse the network. Rewrote the diagnostic experiment in plain language around whether activations remain distinguishable across depth, and rewrote the Level 2 entry without equations so that it follows the observable scale changes of multiplication, addition, and activation. Level 3 retains the variance assumptions, Xavier and He derivations, fan conventions, framework initialisers, and dry-run hooks; Level 4 retains directional spectra, residual and normalised architecture conditions, precision, and audit requirements. Source checks: four level headings; 18 balanced code fences; 5/5 figure captions with alt text; resolving local links and bibliography keys; and a clean whitespace check.
- **Chapter 12 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the variant catalogue with the immediate problem that activation offsets and spreads can change after training starts, even without a layer changing its own parameters. Rewrote normalisation as the choice of a reference set and its resulting information dependencies; moved the group and RMS distinction box from the opening to the Level 2 section that first explains those methods; simplified the reference explanation; and moved synchronised batch-normalisation systems material out of Level 2. Levels 3 and 4 retain reduction-axis notation, framework configuration, dependency gradients, distributed statistics, tensor layout, precision, and audit tests. Source checks: four level headings; 18 balanced code fences; 5/5 figure captions with alt text; resolving local links and bibliography keys; and a clean whitespace check.
- **Chapter 13 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the optimiser catalogue with the immediate gap left after a usable gradient is available: a gradient gives a local direction but not a step size, a treatment of noisy mini-batches, or coordinate-specific scaling. Rewrote the opening around the optimiser and schedule as the procedure that turns those gradients into successive parameter values. Kept the elongated-loss figure as the first concrete case, retained Level 1 for the intuition of momentum, adaptive scaling, and schedules, and kept Levels 2–4 responsible for update state, equations and implementation, and training-system requirements. Moved the L2-versus-decoupled-weight-decay distinction box to the Level 2 explanation where the two mechanisms have first been established. Source checks: four level headings; 18 balanced code fences; 5/5 figure captions with alt text; no local chapter links to resolve; seven resolving bibliography keys; and a clean whitespace check.
- **Chapter 14 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the state-machine opening with the concrete recurrence of the single optimiser update from Chapter 13: the loop decides which observations contribute before the parameters change, when that state is measured, and what survives interruption. Rewrote the lifecycle explanation around the state actually measured, selected, or saved; clarified the competing loop procedures before naming their components; clarified why accumulated micro-batches do not create extra updates; and retained the chapter's distinct level ownership: Level 1 for clocks, phases, and recovery purpose; Level 2 for dependency order and metric, accumulation, mode, and checkpoint mechanics; Level 3 for executable loop and restart-state contracts; and Level 4 for stochastic data loading and systems conditions. Source checks: four level headings; 22 balanced code fences; 7/7 figure captions with alt text; no local chapter links to resolve; five resolving bibliography keys; and a clean whitespace check.
- **Chapter 15 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the generic systems catalogue with the immediate problem that the correct update loop from Chapter 14 can fail or stall on the machine that runs it. Rewrote the opening around distinct observable resource failures, then treated one update as a workload with memory, arithmetic, and communication costs. Rewrote the memory entry to distinguish the parameter file from the memory required by training before presenting the one-billion-parameter accounting figure. Levels 2–4 retain their existing ownership of resource accounting, precision and parallelism mechanics, formulas and profiling, and end-to-end systems and reliability evidence. Source checks: four level headings; 18 balanced code fences; 6/6 figure captions with alt text; no local chapter links to resolve; eight resolving bibliography keys; and a clean whitespace check.
- **Chapter 16 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the Parts I–II overview opening with the small running batch of two sequences, four positions, and three values per position, so the reader meets the represented object before tensor terminology and axis notation. Rewrote the hand-off to explain why earlier model and training operations require these array contracts, made the $B\times S\times D$ notation follow its concrete counterpart, and clarified that “tensor” names a container, not the meaning of its entries. Levels 2–4 retain their existing ownership of named-axis operations, index equations and layout, and evidence for representation quality and operational contracts. Source checks: four level headings; 22 balanced code fences; 7/7 figure captions with alt text; one resolving local chapter link; four resolving bibliography keys; and a clean whitespace check.
- **Chapter 17 — close pedagogical rewrite — completed source pass; rendered review pending.** Replaced the orientation and learning-objective catalogue with the representational problem inherited from Chapter 16: a discrete category can be indexed by an integer without acquiring a meaningful numerical order. Rewrote the opening so an embedding follows as the learned vector that later operations can use, and renamed the constructed-corpus section to explain its specific teaching role. Levels 1–4 retain their existing ownership of lookup and geometry intuition, context-pair and similarity mechanics, predictive objectives and code, and evidence about fitted-space geometry, coverage, bias, evaluation, and systems. Source checks: four level headings; 18 balanced code fences; 7/7 figure captions with alt text; one resolving local chapter link; five resolving prose bibliography keys; and a clean whitespace check. The apparent code-level key `Vt` is a NumPy matrix name, not a citation.
...
- **Chapters 9-12 — generalisation-to-normalisation sequence — first substantive reconstruction — completed source and execution pass.** Replaced the orientation and learning-objective blocks with the next four connected problems: evidence of transfer beyond fitted data (Chapter 9), justified preferences among competing fits (Chapter 10), usable signal and gradient scale before the first update (Chapter 11), and a reference scale for changing intermediate activations (Chapter 12). Added reader guidance before the first capacity, ridge, scale, and reference-axis figure; added explicit intuition-to-mechanics and mechanics-to-mathematics/code transitions; and retained the original technical ownership of evaluation, regularisation mechanisms, initialisation diagnostics, and normalisation axes. Source checks: four level headings per chapter; even code-fence counts (14, 16, 18, and 18); resolving local chapter links; and matching figure captions and alt text (6, 5, 5, and 5). Citation keys resolve; the apparent Chapter 11 `@W` result is matrix notation, not a bibliography citation. Executed and rendered the chapters, generating 6, 5, 5, and 5 PNG figures respectively; published HTML pages include the generated assets.
- **Chapters 13-16 — update-to-representation sequence — first substantive reconstruction — completed source and execution pass.** Replaced the orientation and learning-objective blocks with four connected problems: how gradients become stateful updates (Chapter 13), how updates acquire data, validation, and recovery boundaries (Chapter 14), how a correct loop encounters measured hardware constraints (Chapter 15), and how each operation acts on a numerical representation with named axes (Chapter 16). Added reader guidance before the first optimiser-path, update-lifecycle, resource-diagnosis, and tensor-rank figure; added explicit intuition-to-mechanics and mechanics-to-mathematics/code transitions; and preserved the chapter-specific sequence of optimisation, loop state, systems accounting, and representation contracts. Source checks: four level headings per chapter; even code-fence counts (18, 22, 18, and 22); resolving local chapter links; matching figure captions and alt text (5, 7, 6, and 7); resolving bibliography keys; and a clean whitespace check. Executed and rendered the chapters, generating 5, 7, 6, and 7 PNG figures respectively; published HTML pages include the generated assets.
