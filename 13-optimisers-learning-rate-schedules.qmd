...
Chapter 12 helped ensure that gradients can reach the parameters that need them. A gradient says which small change would reduce the current loss. It does not determine how far to move, how much to trust a gradient from one mini-batch, or whether different parameters should move by the same amount.

An optimiser makes those choices from the current gradient and, in many cases, a record of earlier ones. A learning-rate schedule changes the overall size of the updates as training progresses. Together they turn a sequence of gradients into a sequence of parameter values.
