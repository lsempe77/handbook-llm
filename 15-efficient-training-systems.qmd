...
A loop can be correct and still be impossible to run at the required scale. The model or its activations may not fit in memory; arithmetic units may wait for data; workers may wait for collective communication; or a pipeline stage may be idle. These are different measured limits. Adding devices does not repair a memory-bound operation, and lower precision does not eliminate an unsuitable interconnect.

This final chapter of Part II treats one update as a systems workload: parameters, gradients, optimiser state, activations, temporary buffers, arithmetic, memory traffic, and communication all contribute. Mixed precision, fusion, activation recomputation, parallelism, and sharding each change a particular cost and introduce a particular dependency. Measurement determines which intervention has a plausible mechanism.
