...
Chapter 9 showed why a flexible fitted rule can follow sample-specific noise. Reducing the model's nominal capacity is one response, but it is not the only one. Regularisation changes the learning procedure so that, among available fits, some are preferred: a penalty can make large parameters costly; dropout can train changing subnetworks; augmentation can add transformations judged label-preserving; and early stopping can select an earlier checkpoint.

None of these operations repairs an invalid label, a leaking split, or a population absent from the evaluation design. Each adds an assumption about the fitted solution, the data, or the optimisation path. The method and its strength must consequently be chosen using validation evidence and assessed against the intended transfer task.
...
The figure compares two rules with the same input features and polynomial degree. The coloured curve changes because the objective changes: the ridge fit is permitted to accept more error on the observed points when that avoids extreme coefficients. This is a preference about solutions, not a claim that smoothness is always correct.
