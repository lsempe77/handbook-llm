...
Chapter 8 trained a network until it classified a synthetic training set and a held-out set correctly. Because that held-out result was inspected throughout training, it functioned as validation evidence rather than as an untouched final test. The result also depended on a simple data-generating process, a favourable split, and a model suited to the pattern. In applied work, the training sample is finite, labels contain error, and future observations may differ from those used to fit the model.

Generalisation describes performance on observations not used to estimate or select the model. Overfitting occurs when a fitted model captures idiosyncrasies of the training sample that do not transfer to the target population. A low training loss is therefore evidence about optimisation on observed data. It is not, by itself, evidence about future performance.

### Learning objectives

By the end of this chapter, you should be able to:

- distinguish training fit, held-out performance, and target-population performance;
- identify underfitting and overfitting from fitted functions and error curves;
- explain the separate roles of training, validation, and test data;
- define empirical risk, population risk, and the generalisation gap;
- describe bias and variance without treating them as fixed properties of an algorithm;
- implement capacity and sample-size experiments in Python;
- explain why one favourable split is uncertain evidence;
- distinguish interpolation from generalisation;
- explain how test-set reuse, contamination, and distribution shift weaken performance claims.
...
The practical conclusion at this level is limited: training performance must be compared with genuinely unseen data, and the comparison must match the population or conditions for which the model is claimed to work. Level 2 shows how the data and diagnostics are organised.
...
Level 2 uses held-out errors as diagnostics. Level 3 defines the quantities those diagnostics estimate and decomposes their sampling behaviour.
