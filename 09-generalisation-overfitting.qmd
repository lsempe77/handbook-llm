...
The network in Chapter 8 learned a boundary that followed the training curves. It also looked promising on the observations set aside from those updates. The natural next question is whether that result would persist for other observations, rather than being a fortunate feature of this particular sample.

This is the problem of generalisation. A fitted rule can improve its training loss by learning a pattern that recurs, or by following details that arose only in the observations it happened to receive. The second outcome is overfitting. Training loss records what happened on the data used for updates; it cannot on its own tell us which kind of improvement occurred.
...
**Generalisation and overfitting.** Generalisation is performance on new observations from a stated target setting. Overfitting occurs when a training improvement depends on details that do not recur. Training loss alone cannot distinguish them.
:::
...
### New observations test whether the pattern carries over

To learn whether the fitted rule carries over, we need observations whose outcomes did not help to choose it. Holding out observations provides that first check. If the model performs similarly on them, the result is evidence that the pattern extends beyond the training sample.

That evidence has limits. A small holdout can be unusually easy or hard, and a random split can place near-duplicates on both sides. The next level separates the roles of training, validation, and final test observations.
...
The immediate task is to compare performance on observations used for fitting with performance on observations saved for evaluation. Level 2 makes the different roles explicit, then uses capacity and sample-size curves to diagnose the gap.
...
**Training, validation, and test sets.** Training data estimate parameters. Validation data support choices among procedures or checkpoints and therefore belong to development. A test set is reserved for final assessment after those choices are fixed. A **held-out** set is excluded from one fitting or selection stage; it is not automatically an untouched final test set.
:::
