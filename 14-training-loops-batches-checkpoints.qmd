...
Chapter 13 described one optimiser update: a gradient and the optimiser's current state produce a new set of parameters. Training requires that update to be repeated over data. The surrounding loop determines which observations contribute before the update, when the parameters are changed, when their current performance is measured, and what can be recovered after an interruption.

Two scripts can share an architecture, loss, and optimiser while still fitting different procedures. One may update after every micro-batch; another may accumulate gradients across several. One may select a checkpoint from validation results; another may save only the most recent weights. This chapter follows those choices through data loading, batching, evaluation, logging, checkpoint selection, and restart.

### One update inside its lifecycle

Read the diagram from left to right. A backward pass can add gradients to an accumulation window, but an optimiser update happens only at its declared boundary. Validation and checkpointing run less often, yet each refers to the parameter state produced at one of those boundaries. The timing therefore determines which state was measured, selected, or saved.
...
An epoch is one traversal of the training dataset under a specified sampler. An update is one parameter change. These clocks are related but not interchangeable. With 10,000 observations, an effective batch of 100 gives approximately 100 updates per epoch. Processing micro-batches of 25 with four-way accumulation still gives approximately 100 updates, because four passes contribute to each parameter change.
