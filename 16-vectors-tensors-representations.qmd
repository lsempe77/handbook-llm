...
Parts I and II defined models, gradients, updates, and the systems that execute them. Every one of those operations acts on numerical arrays. The values alone do not identify what an axis means, which entries belong to one observation, or whether two coordinates can be compared. Those meanings come from the representation and the operations that produce it.

This chapter begins Part III by making those array contracts explicit before the book turns to embeddings and sequences. It separates rank from vector dimension, attaches a meaning to each axis, and follows how transformations preserve or discard distinctions. A tensor can have a valid shape while carrying the wrong semantics for the operation applied to it.
