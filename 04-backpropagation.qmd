...
Gradient descent can update a parameter once it has that parameter's gradient. A neural network may have millions of parameters, each affecting the loss through a long sequence of operations. Calculating a separate derivative from scratch for every parameter would make training impractical.

Backpropagation reuses the computations that produced the loss. It first records a forward calculation, then moves backwards through that calculation, passing a local sensitivity from each operation to the values that produced it.
...
The figure separates two jobs that are often collapsed into “training”. The upper row computes a prediction and then a loss. The lower row answers a different question: how would a small change in an earlier value alter that final loss? Follow the upper row first; the backward arrows will then have something concrete to refer to.
...
The forward pass has supplied the values at each operation. The remaining task is to reuse those values to calculate a gradient for every earlier adjustable quantity, without repeating the entire computation once per parameter.
...
We can now distinguish the two operations behind ordinary neural-network training: backpropagation calculates gradients, and gradient descent uses them to update parameters. [Chapter 5](05-artificial-neuron.qmd) introduces the smallest parameterised building block to which those operations are applied: the artificial neuron.
