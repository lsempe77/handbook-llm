...
Chapters 1–7 introduced the parts of a neural network separately. We now have data, layers, activations, a loss, a way to calculate gradients, and an optimiser. None of those parts learns on its own. They must be connected in an order that lets an earlier prediction change the parameters used for the next one.

This chapter joins them in one small binary classifier. The data are deliberately simple: two curved groups that one straight boundary cannot separate. They make the training sequence visible before the book turns to the harder question of whether a fitted model will work beyond the records used to train it.
...
The outlined observations remain outside training. They let us see whether the final boundary extends to records that did not contribute to the updates. The split is for this illustration; Chapter 9 develops the evidence required to assess generalisation.

## Level 1 - Intuition

The training loop is where the earlier chapters become one procedure. Follow one cycle before reading the implementation: take a batch, calculate scores, compare them with the recorded outcomes, send the resulting information backwards, and change the weights used for the next batch.
...
Each box has one role. The batch supplies inputs and recorded outcomes. The forward pass produces scores. The loss compares those scores with the outcomes. Backpropagation calculates how each parameter contributed to that loss, and the optimiser changes the parameters. Reversing these dependencies would change the algorithm.

::: {.concept-box .concept-core #concept-training-loop data-term="Training loop" data-domain="Foundations"}
**Training loop.** The repeated sequence that takes a batch, makes predictions, calculates their loss, traces gradients backwards, and updates the model's parameters.
:::
...
The final layer produces one score. In the next level, that score will be converted into a probability and compared with the known class. For now, the important point is that the network can bend its boundary because the hidden layer uses a non-linear activation.

Level 2 follows one observation through the operations that the loop repeats.
...
Level 2 followed one observation through one update. Actual code carries out the same chain for a batch, keeps gradients and optimiser state separately, and evaluates the trained model without changing it.
